import os
import sqlite3
import logging
import json
import time
import hmac
import hashlib
import urllib.parse
import threading

from datetime import datetime
from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer

from telegram import (
    Update,
    InlineKeyboardButton,
    InlineKeyboardMarkup,
    WebAppInfo,
)
from telegram.ext import (
    Application,
    CommandHandler,
    CallbackQueryHandler,
    MessageHandler,
    ContextTypes,
    filters,
)

# =========================================================
# CONFIG
# =========================================================

BOT_TOKEN = os.getenv("BOT_TOKEN", "").strip()

OWNER_ID = 8552447077

MINI_APP_URL = os.getenv(
    "MINI_APP_URL",
    "https://mmadpnl.github.io/-/"
).strip()

DB_FILE = os.getenv(
    "DB_FILE",
    "limbo.db"
)

DEPOSIT_WALLET = "@IQ7XA"
DEPOSIT_PREFIX = "ULTRA"

# پورت API
PORT = int(os.getenv("PORT", "10000"))

# حداکثر عمر initData تلگرام
WEBAPP_AUTH_MAX_AGE = 86400


# =========================================================
# LOGGING
# =========================================================

logging.basicConfig(
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    level=logging.INFO,
)

logger = logging.getLogger("limbo")


# =========================================================
# DATABASE
# =========================================================

def db():
    conn = sqlite3.connect(
        DB_FILE,
        timeout=30,
        check_same_thread=False
    )

    conn.row_factory = sqlite3.Row

    return conn


def init_db():

    conn = db()

    conn.execute("""
        CREATE TABLE IF NOT EXISTS users (
            user_id INTEGER PRIMARY KEY,
            username TEXT DEFAULT '',
            first_name TEXT DEFAULT '',
            balance REAL DEFAULT 0,
            created_at TEXT
        )
    """)

    conn.execute("""
        CREATE TABLE IF NOT EXISTS requests (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id INTEGER NOT NULL,
            type TEXT NOT NULL,
            amount REAL NOT NULL,
            wallet TEXT DEFAULT '',
            username TEXT DEFAULT '',
            proof TEXT DEFAULT '',
            status TEXT DEFAULT 'pending',
            created_at TEXT
        )
    """)

    conn.execute("""
        CREATE TABLE IF NOT EXISTS settings (
            key TEXT PRIMARY KEY,
            value TEXT
        )
    """)

    conn.execute("""
        INSERT OR IGNORE INTO settings(key,value)
        VALUES('bot_enabled','1')
    """)

    conn.commit()
    conn.close()

    logger.info("Database initialized")


def register_user(user):

    if not user:
        return

    conn = db()

    conn.execute("""
        INSERT INTO users
        (
            user_id,
            username,
            first_name,
            balance,
            created_at
        )
        VALUES (?, ?, ?, 0, ?)

        ON CONFLICT(user_id)
        DO UPDATE SET
            username=excluded.username,
            first_name=excluded.first_name
    """, (
        user.id,
        user.username or "",
        user.first_name or "",
        datetime.now().isoformat()
    ))

    conn.commit()
    conn.close()


def user_exists(user_id):

    conn = db()

    row = conn.execute(
        "SELECT user_id FROM users WHERE user_id=?",
        (user_id,)
    ).fetchone()

    conn.close()

    return row is not None


def get_balance(user_id):

    conn = db()

    row = conn.execute(
        """
        SELECT balance
        FROM users
        WHERE user_id=?
        """,
        (int(user_id),)
    ).fetchone()

    conn.close()

    if not row:
        return 0.0

    try:
        return float(row["balance"])
    except Exception:
        return 0.0


def change_balance(user_id, amount):

    conn = db()

    try:

        row = conn.execute(
            """
            SELECT balance
            FROM users
            WHERE user_id=?
            """,
            (int(user_id),)
        ).fetchone()

        if not row:
            conn.close()
            return False

        current = float(row["balance"] or 0)

        new_balance = current + float(amount)

        if new_balance < 0:
            conn.close()
            return False

        conn.execute(
            """
            UPDATE users
            SET balance=?
            WHERE user_id=?
            """,
            (
                round(new_balance, 2),
                int(user_id)
            )
        )

        conn.commit()

        conn.close()

        return True

    except Exception:

        conn.rollback()
        conn.close()

        logger.exception("change_balance error")

        return False


def bot_enabled():

    conn = db()

    row = conn.execute(
        """
        SELECT value
        FROM settings
        WHERE key='bot_enabled'
        """
    ).fetchone()

    conn.close()

    return bool(
        row and row["value"] == "1"
    )


def set_bot_enabled(value):

    conn = db()

    conn.execute(
        """
        UPDATE settings
        SET value=?
        WHERE key='bot_enabled'
        """,
        ("1" if value else "0",)
    )

    conn.commit()
    conn.close()


# =========================================================
# OWNER
# =========================================================

def is_owner(user_id):

    return int(user_id) == OWNER_ID


# =========================================================
# TELEGRAM WEB APP INIT DATA VALIDATION
# =========================================================

def validate_webapp_init_data(init_data):

    if not BOT_TOKEN:
        return None

    if not init_data:
        return None

    try:

        parsed = urllib.parse.parse_qs(
            init_data,
            keep_blank_values=True
        )

        received_hash_list = parsed.get("hash")

        if not received_hash_list:
            return None

        received_hash = received_hash_list[0]

        data = {}

        for key, values in parsed.items():

            if key == "hash":
                continue

            if not values:
                continue

            data[key] = values[0]

        auth_date = int(
            data.get("auth_date", "0")
        )

        now = int(time.time())

        if auth_date <= 0:
            return None

        if now - auth_date > WEBAPP_AUTH_MAX_AGE:
            logger.warning(
                "WebApp initData expired"
            )
            return None

        data_check_string = "\n".join(
            f"{key}={data[key]}"
            for key in sorted(data)
        )

        secret_key = hmac.new(
            b"WebAppData",
            BOT_TOKEN.encode(),
            hashlib.sha256
        ).digest()

        calculated_hash = hmac.new(
            secret_key,
            data_check_string.encode(),
            hashlib.sha256
        ).hexdigest()

        if not hmac.compare_digest(
            calculated_hash,
            received_hash
        ):
            logger.warning(
                "Invalid WebApp hash"
            )
            return None

        user_json = data.get("user")

        if not user_json:
            return None

        user = json.loads(user_json)

        user_id = int(user["id"])

        return {
            "user_id": user_id,
            "username": user.get(
                "username",
                ""
            ),
            "first_name": user.get(
                "first_name",
                ""
            )
        }

    except Exception:

        logger.exception(
            "WebApp validation error"
        )

        return None


# =========================================================
# MINI APP API
# =========================================================

class APIHandler(BaseHTTPRequestHandler):

    def log_message(
        self,
        format,
        *args
    ):
        return

    def send_json(
        self,
        status,
        data
    ):

        body = json.dumps(
            data,
            ensure_ascii=False
        ).encode("utf-8")

        self.send_response(status)

        self.send_header(
            "Content-Type",
            "application/json; charset=utf-8"
        )

        self.send_header(
            "Content-Length",
            str(len(body))
        )

        # برای GitHub Pages
        self.send_header(
            "Access-Control-Allow-Origin",
            "*"
        )

        self.send_header(
            "Access-Control-Allow-Headers",
            "Content-Type, Authorization"
        )

        self.send_header(
            "Access-Control-Allow-Methods",
            "GET, OPTIONS"
        )

        self.end_headers()

        self.wfile.write(body)

    def do_OPTIONS(self):

        self.send_response(204)

        self.send_header(
            "Access-Control-Allow-Origin",
            "*"
        )

        self.send_header(
            "Access-Control-Allow-Headers",
            "Content-Type, Authorization"
        )

        self.send_header(
            "Access-Control-Allow-Methods",
            "GET, OPTIONS"
        )

        self.end_headers()

    def do_GET(self):

        try:

            parsed = urllib.parse.urlparse(
                self.path
            )

            path = parsed.path

            query = urllib.parse.parse_qs(
                parsed.query
            )

            # ---------------------------------------------
            # HEALTH
            # ---------------------------------------------

            if path == "/":

                self.send_json(
                    200,
                    {
                        "ok": True,
                        "service": "LIMBO DOGS API"
                    }
                )

                return

            # ---------------------------------------------
            # HEALTH
            # ---------------------------------------------

            if path == "/health":

                self.send_json(
                    200,
                    {
                        "ok": True
                    }
                )

                return

            # ---------------------------------------------
            # BALANCE
            # ---------------------------------------------

            if path == "/api/balance":

                init_data_list = query.get(
                    "initData"
                )

                if not init_data_list:

                    self.send_json(
                        401,
                        {
                            "ok": False,
                            "error": "missing_initData"
                        }
                    )

                    return

                init_data = init_data_list[0]

                web_user = validate_webapp_init_data(
                    init_data
                )

                if not web_user:

                    self.send_json(
                        401,
                        {
                            "ok": False,
                            "error": "invalid_initData"
                        }
                    )

                    return

                user_id = web_user["user_id"]

                # ثبت کاربر در DB اگر وجود نداشته باشد
                conn = db()

                conn.execute(
                    """
                    INSERT OR IGNORE INTO users
                    (
                        user_id,
                        username,
                        first_name,
                        balance,
                        created_at
                    )
                    VALUES (?, ?, ?, 0, ?)
                    """,
                    (
                        user_id,
                        web_user.get(
                            "username",
                            ""
                        ),
                        web_user.get(
                            "first_name",
                            ""
                        ),
                        datetime.now().isoformat()
                    )
                )

                conn.commit()
                conn.close()

                balance = get_balance(
                    user_id
                )

                self.send_json(
                    200,
                    {
                        "ok": True,
                        "user_id": user_id,
                        "balance": balance,
                        "currency": "DOGS"
                    }
                )

                return

            # ---------------------------------------------
            # NOT FOUND
            # ---------------------------------------------

            self.send_json(
                404,
                {
                    "ok": False,
                    "error": "not_found"
                }
            )

        except Exception:

            logger.exception(
                "API error"
            )

            try:

                self.send_json(
                    500,
                    {
                        "ok": False,
                        "error": "server_error"
                    }
                )

            except Exception:
                pass


def start_api_server():

    try:

        server = ThreadingHTTPServer(
            ("0.0.0.0", PORT),
            APIHandler
        )

        logger.info(
            "API SERVER RUNNING ON PORT %s",
            PORT
        )

        server.serve_forever()

    except Exception:

        logger.exception(
            "API server failed"
        )


# =========================================================
# KEYBOARDS
# =========================================================

def main_keyboard(user_id=None):

    buttons = [
        [
            InlineKeyboardButton(
                "🎮 LIMBO",
                web_app=WebAppInfo(
                    url=MINI_APP_URL
                )
            )
        ],
        [
            InlineKeyboardButton(
                "🐶 موجودی",
                callback_data="balance"
            ),
            InlineKeyboardButton(
                "💳 واریز",
                callback_data="deposit"
            )
        ],
        [
            InlineKeyboardButton(
                "💸 برداشت",
                callback_data="withdraw"
            )
        ],
        [
            InlineKeyboardButton(
                "📞 تماس با ما",
                callback_data="support"
            )
        ]
    ]

    if (
        user_id is not None
        and is_owner(user_id)
    ):

        buttons.append([
            InlineKeyboardButton(
                "👑 پنل مدیریت",
                callback_data="admin"
            )
        ])

    return InlineKeyboardMarkup(
        buttons
    )


def cancel_keyboard():

    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                "❌ لغو",
                callback_data="cancel"
            )
        ]
    ])


def admin_keyboard():

    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                "💳 واریزی‌ها",
                callback_data="admin_deposits"
            )
        ],
        [
            InlineKeyboardButton(
                "💸 برداشت‌ها",
                callback_data="admin_withdrawals"
            )
        ],
        [
            InlineKeyboardButton(
                "➕ شارژ موجودی",
                callback_data="admin_add"
            ),
            InlineKeyboardButton(
                "➖ کسر موجودی",
                callback_data="admin_remove"
            )
        ],
        [
            InlineKeyboardButton(
                "📞 تماس با کاربر",
                callback_data="admin_contact"
            )
        ],
        [
            InlineKeyboardButton(
                "👑 انتقال مالکیت",
                callback_data="admin_transfer"
            )
        ],
        [
            InlineKeyboardButton(
                "🔄 روشن / خاموش",
                callback_data="admin_toggle"
            )
        ],
        [
            InlineKeyboardButton(
                "🔙 برگشت",
                callback_data="back"
            )
        ]
    ])


async def safe_edit(
    query,
    text,
    keyboard=None
):

    try:

        await query.edit_message_text(
            text,
            reply_markup=keyboard
        )

    except Exception:

        try:

            await query.message.reply_text(
                text,
                reply_markup=keyboard
            )

        except Exception:
            pass


# =========================================================
# STATES
# =========================================================

states = {}


# =========================================================
# START
# =========================================================

async def start(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE
):

    user = update.effective_user

    register_user(user)

    if (
        not bot_enabled()
        and not is_owner(user.id)
    ):

        await update.message.reply_text(
            "⛔ ربات موقتاً خاموش است."
        )

        return

    balance = get_balance(
        user.id
    )

    await update.message.reply_text(
        f"🐶 **LIMBO DOGS**\n\n"
        f"💰 موجودی:\n"
        f"`{balance:,.2f} DOGS`\n\n"
        f"🎮 برای بازی روی LIMBO بزنید.",
        parse_mode="Markdown",
        reply_markup=main_keyboard(
            user.id
        )
    )


# =========================================================
# ADMIN COMMAND
# =========================================================

async def admin_command(
    update,
    context
):

    if not is_owner(
        update.effective_user.id
    ):

        await update.message.reply_text(
            "⛔ دسترسی ندارید."
        )

        return

    await update.message.reply_text(
        "👑 **پنل مدیریت**",
        parse_mode="Markdown",
        reply_markup=admin_keyboard()
    )


# =========================================================
# CALLBACKS
# =========================================================

async def callbacks(
    update,
    context
):

    query = update.callback_query

    await query.answer()

    user = query.from_user

    register_user(user)

    data = query.data

    # -----------------------------------------------------
    # CANCEL
    # -----------------------------------------------------

    if data == "cancel":

        states.pop(
            user.id,
            None
        )

        await safe_edit(
            query,
            "❌ عملیات لغو شد.",
            main_keyboard(user.id)
        )

        return

    # -----------------------------------------------------
    # BACK
    # -----------------------------------------------------

    if data == "back":

        states.pop(
            user.id,
            None
        )

        if is_owner(user.id):

            await safe_edit(
                query,
                "👑 **پنل مدیریت**",
                admin_keyboard()
            )

        else:

            await safe_edit(
                query,
                "🏠 منوی اصلی",
                main_keyboard(user.id)
            )

        return

    # -----------------------------------------------------
    # BALANCE
    # -----------------------------------------------------

    if data == "balance":

        balance = get_balance(
            user.id
        )

        await safe_edit(
            query,
            f"🐶 **موجودی شما**\n\n"
            f"💰 `{balance:,.2f} DOGS`",
            main_keyboard(user.id)
        )

        return

    # -----------------------------------------------------
    # DEPOSIT
    # -----------------------------------------------------

    if data == "deposit":

        states[user.id] = {
            "action": "deposit_amount"
        }

        await safe_edit(
            query,
            "💳 **واریز DOGS**\n\n"
            "تعداد DOGS را وارد کنید.\n\n"
            "مثال:\n"
            "`5000`",
            cancel_keyboard()
        )

        return

    # -----------------------------------------------------
    # WITHDRAW
    # -----------------------------------------------------

    if data == "withdraw":

        balance = get_balance(
            user.id
        )

        states[user.id] = {
            "action": "withdraw_amount"
        }

        await safe_edit(
            query,
            f"💸 **برداشت DOGS**\n\n"
            f"موجودی شما:\n"
            f"`{balance:,.2f} DOGS`\n\n"
            "تعداد برداشت را وارد کنید:",
            cancel_keyboard()
        )

        return

    # -----------------------------------------------------
    # SUPPORT
    # -----------------------------------------------------

    if data == "support":

        states[user.id] = {
            "action": "user_support"
        }

        await safe_edit(
            query,
            "📞 **تماس با ما**\n\n"
            "پیام خود را ارسال کنید.",
            cancel_keyboard()
        )

        return

    # =====================================================
    # ADMIN
    # =====================================================

    if data == "admin":

        if not is_owner(user.id):
            return

        status = (
            "🟢 روشن"
            if bot_enabled()
            else
            "🔴 خاموش"
        )

        await safe_edit(
            query,
            f"👑 **پنل مدیریت**\n\n"
            f"وضعیت: {status}",
            admin_keyboard()
        )

        return

    # -----------------------------------------------------
    # ADMIN DEPOSITS
    # -----------------------------------------------------

    if data == "admin_deposits":

        if not is_owner(user.id):
            return

        await admin_requests(
            query,
            "deposit"
        )

        return

    # -----------------------------------------------------
    # ADMIN WITHDRAWALS
    # -----------------------------------------------------

    if data == "admin_withdrawals":

        if not is_owner(user.id):
            return

        await admin_requests(
            query,
            "withdraw"
        )

        return

    # -----------------------------------------------------
    # ADMIN CONTACT
    # -----------------------------------------------------

    if data == "admin_contact":

        if not is_owner(user.id):
            return

        states[user.id] = {
            "action": "admin_contact_target"
        }

        await safe_edit(
            query,
            "📞 **تماس با کاربر**\n\n"
            "ID عددی کاربر را وارد کنید:",
            cancel_keyboard()
        )

        return

    # -----------------------------------------------------
    # ADMIN TOGGLE
    # -----------------------------------------------------

    if data == "admin_toggle":

        if not is_owner(user.id):
            return

        current = bot_enabled()

        set_bot_enabled(
            not current
        )

        status = (
            "🟢 روشن"
            if not current
            else
            "🔴 خاموش"
        )

        await safe_edit(
            query,
            f"وضعیت ربات:\n\n{status}",
            admin_keyboard()
        )

        return

    # -----------------------------------------------------
    # ADMIN ADD
    # -----------------------------------------------------

    if data == "admin_add":

        if not is_owner(user.id):
            return

        states[user.id] = {
            "action": "admin_add_user"
        }

        await safe_edit(
            query,
            "➕ **شارژ موجودی**\n\n"
            "ID عددی کاربر را وارد کنید:",
            cancel_keyboard()
        )

        return

    # -----------------------------------------------------
    # ADMIN REMOVE
    # -----------------------------------------------------

    if data == "admin_remove":

        if not is_owner(user.id):
            return

        states[user.id] = {
            "action": "admin_remove_user"
        }

        await safe_edit(
            query,
            "➖ **کسر موجودی**\n\n"
            "ID عددی کاربر را وارد کنید:",
            cancel_keyboard()
        )

        return

    # -----------------------------------------------------
    # ADMIN TRANSFER
    # -----------------------------------------------------

    if data == "admin_transfer":

        if not is_owner(user.id):
            return

        states[user.id] = {
            "action": "transfer_owner"
        }

        await safe_edit(
            query,
            "👑 **انتقال مالکیت**\n\n"
            "ID عددی مالک جدید را وارد کنید:",
            cancel_keyboard()
        )

        return

    # -----------------------------------------------------
    # APPROVE
    # -----------------------------------------------------

    if data.startswith("approve_"):

        if not is_owner(user.id):
            return

        try:

            request_id = int(
                data.split("_", 1)[1]
            )

        except Exception:

            return

        await process_request(
            query,
            request_id,
            True
        )

        return

    # -----------------------------------------------------
    # REJECT
    # -----------------------------------------------------

    if data.startswith("reject_"):

        if not is_owner(user.id):
            return

        try:

            request_id = int(
                data.split("_", 1)[1]
            )

        except Exception:

            return

        await process_request(
            query,
            request_id,
            False
        )

        return


# =========================================================
# ADMIN REQUESTS
# =========================================================

async def admin_requests(
    query,
    request_type
):

    conn = db()

    rows = conn.execute(
        """
        SELECT *
        FROM requests
        WHERE type=?
        AND status='pending'
        ORDER BY id DESC
        LIMIT 20
        """,
        (request_type,)
    ).fetchall()

    conn.close()

    if request_type == "deposit":

        title = "💳 **واریزی‌های در انتظار**"

    else:

        title = "💸 **برداشت‌های در انتظار**"

    if not rows:

        await safe_edit(
            query,
            f"{title}\n\nموردی وجود ندارد.",
            admin_keyboard()
        )

        return

    text = title + "\n\n"

    buttons = []

    for row in rows:

        text += (
            f"🆔 درخواست: `#{row['id']}`\n"
            f"👤 ID: `{row['user_id']}`\n"
            f"👤 @{row['username'] or 'ندارد'}\n"
            f"🐶 مبلغ: `{row['amount']:,.2f} DOGS`\n"
        )

        if row["proof"]:

            text += (
                f"📎 مدرک: {row['proof']}\n"
            )

        text += "\n"

        buttons.append([
            InlineKeyboardButton(
                f"✅ تأیید #{row['id']}",
                callback_data=f"approve_{row['id']}"
            ),
            InlineKeyboardButton(
                f"❌ رد #{row['id']}",
                callback_data=f"reject_{row['id']}"
            )
        ])

    buttons.append([
        InlineKeyboardButton(
            "🔙 برگشت",
            callback_data="admin"
        )
    ])

    await safe_edit(
        query,
        text,
        InlineKeyboardMarkup(
            buttons
        )
    )


# =========================================================
# PROCESS REQUEST
# =========================================================

async def process_request(
    query,
    request_id,
    approve
):

    conn = db()

    row = conn.execute(
        """
        SELECT *
        FROM requests
        WHERE id=?
        """,
        (request_id,)
    ).fetchone()

    if not row:

        conn.close()

        await safe_edit(
            query,
            "❌ درخواست پیدا نشد.",
            admin_keyboard()
        )

        return

    if row["status"] != "pending":

        conn.close()

        await safe_edit(
            query,
            "⚠️ این درخواست قبلاً بررسی شده.",
            admin_keyboard()
        )

        return

    # -----------------------------------------------------
    # APPROVE
    # -----------------------------------------------------

    if approve:

        if row["type"] == "deposit":

            success = change_balance(
                row["user_id"],
                row["amount"]
            )

        else:

            success = change_balance(
                row["user_id"],
                -row["amount"]
            )

        if not success:

            conn.close()

            await safe_edit(
                query,
                "❌ عملیات انجام نشد؛ موجودی کافی نیست یا کاربر وجود ندارد.",
                admin_keyboard()
            )

            return

        conn.execute(
            """
            UPDATE requests
            SET status='approved'
            WHERE id=?
            """,
            (request_id,)
        )

        result = "approved"

    # -----------------------------------------------------
    # REJECT
    # -----------------------------------------------------

    else:

        conn.execute(
            """
            UPDATE requests
            SET status='rejected'
            WHERE id=?
            """,
            (request_id,)
        )

        result = "rejected"

    conn.commit()
    conn.close()

    # -----------------------------------------------------
    # USER NOTIFICATION
    # -----------------------------------------------------

    try:

        if result == "approved":

            if row["type"] == "deposit":

                new_balance = get_balance(
                    row["user_id"]
                )

                msg = (
                    "✅ **واریز تأیید شد**\n\n"
                    f"🐶 +{row['amount']:,.2f} DOGS\n"
                    f"💰 موجودی جدید: "
                    f"{new_balance:,.2f} DOGS"
                )

            else:

                new_balance = get_balance(
                    row["user_id"]
                )

                msg = (
                    "✅ **برداشت تأیید شد**\n\n"
                    f"🐶 {row['amount']:,.2f} DOGS\n"
                    f"💰 موجودی باقی‌مانده: "
                    f"{new_balance:,.2f} DOGS"
                )

        else:

            msg = (
                "❌ **درخواست رد شد**\n\n"
                f"🐶 {row['amount']:,.2f} DOGS"
            )

        await query.get_bot().send_message(
            row["user_id"],
            msg,
            parse_mode="Markdown"
        )

    except Exception as e:

        logger.error(
            "Notification error: %s",
            e
        )

    status_text = (
        "✅ تأیید شد"
        if result == "approved"
        else
        "❌ رد شد"
    )

    await safe_edit(
        query,
        f"درخواست `#{request_id}` {status_text}.",
        admin_keyboard()
    )


# =========================================================
# TEXT HANDLER
# =========================================================

async def text_handler(
    update,
    context
):

    user = update.effective_user

    register_user(user)

    text = (
        update.message.text or ""
    ).strip()

    state = states.get(
        user.id
    )

    if not state:

        await update.message.reply_text(
            "از دکمه‌های ربات استفاده کنید.",
            reply_markup=main_keyboard(
                user.id
            )
        )

        return

    action = state.get(
        "action"
    )

    # =====================================================
    # DEPOSIT AMOUNT
    # =====================================================

    if action == "deposit_amount":

        try:

            amount = float(
                text.replace(",", "")
            )

            if amount <= 0:
                raise ValueError

        except Exception:

            await update.message.reply_text(
                "❌ فقط عدد معتبر وارد کنید.\n\n"
                "مثال:\n"
                "`5000`",
                parse_mode="Markdown",
                reply_markup=cancel_keyboard()
            )

            return

        deposit_format = (
            f"{DEPOSIT_PREFIX} "
            f"{amount:g} DOGS "
            f"{DEPOSIT_WALLET}"
        )

        states[user.id] = {
            "action": "deposit_proof",
            "amount": amount,
            "deposit_format": deposit_format
        }

        await update.message.reply_text(
            f"💳 **واریز DOGS**\n\n"
            f"مبلغ انتخابی:\n"
            f"🐶 `{amount:,.2f} DOGS`\n\n"
            f"📋 **فرمت واریز:**\n"
            f"`{deposit_format}`\n\n"
            f"بعد از واریز، یکی از موارد زیر را همینجا ارسال کنید:\n"
            f"• 📸 شات\n"
            f"• 🔗 لینک تراکنش\n"
            f"• 💬 پیام واریزی\n\n"
            f"⏳ سپس مالک درخواست را بررسی می‌کند.",
            parse_mode="Markdown",
            reply_markup=cancel_keyboard()
        )

        return

    # =====================================================
    # DEPOSIT PROOF
    # =====================================================

    if action == "deposit_proof":

        amount = state["amount"]

        deposit_format = state[
            "deposit_format"
        ]

        conn = db()

        conn.execute(
            """
            INSERT INTO requests
            (
                user_id,
                type,
                amount,
                username,
                proof,
                status,
                created_at
            )
            VALUES (
                ?,
                'deposit',
                ?,
                ?,
                ?,
                'pending',
                ?
            )
            """,
            (
                user.id,
                amount,
                user.username or "",
                text,
                datetime.now().isoformat()
            )
        )

        conn.commit()

        request_id = conn.execute(
            "SELECT last_insert_rowid()"
        ).fetchone()[0]

        conn.close()

        states.pop(
            user.id,
            None
        )

        await update.message.reply_text(
            f"✅ **درخواست واریز ثبت شد**\n\n"
            f"🐶 مبلغ: `{amount:,.2f} DOGS`\n"
            f"📋 فرمت: `{deposit_format}`\n"
            f"🆔 درخواست: `#{request_id}`\n\n"
            f"⏳ در انتظار بررسی مالک.",
            parse_mode="Markdown",
            reply_markup=main_keyboard(
                user.id
            )
        )

        await send_owner_request(
            context,
            request_id,
            "deposit",
            user,
            amount,
            text
        )

        return

    # =====================================================
    # WITHDRAW AMOUNT
    # =====================================================

    if action == "withdraw_amount":

        try:

            amount = float(
                text.replace(",", "")
            )

            if amount <= 0:
                raise ValueError

        except Exception:

            await update.message.reply_text(
                "❌ فقط عدد معتبر وارد کنید.",
                reply_markup=cancel_keyboard()
            )

            return

        balance = get_balance(
            user.id
        )

        if amount > balance:

            await update.message.reply_text(
                f"❌ موجودی کافی نیست.\n\n"
                f"💰 موجودی:\n"
                f"`{balance:,.2f} DOGS`",
                parse_mode="Markdown",
                reply_markup=cancel_keyboard()
            )

            return

        states[user.id] = {
            "action": "withdraw_info",
            "amount": amount
        }

        await update.message.reply_text(
            "💸 **برداشت DOGS**\n\n"
            "ID و ولت مقصد را ارسال کنید.\n\n"
            "مثال:\n"
            "`ID: 123456789`\n"
            "`Wallet: UQ...`",
            parse_mode="Markdown",
            reply_markup=cancel_keyboard()
        )

        return

    # =====================================================
    # WITHDRAW INFO
    # =====================================================

    if action == "withdraw_info":

        amount = state["amount"]

        conn = db()

        conn.execute(
            """
            INSERT INTO requests
            (
                user_id,
                type,
                amount,
                username,
                proof,
                status,
                created_at
            )
            VALUES (
                ?,
                'withdraw',
                ?,
                ?,
                ?,
                'pending',
                ?
            )
            """,
            (
                user.id,
                amount,
                user.username or "",
                text,
                datetime.now().isoformat()
            )
        )

        conn.commit()

        request_id = conn.execute(
            "SELECT last_insert_rowid()"
        ).fetchone()[0]

        conn.close()

        states.pop(
            user.id,
            None
        )

        await update.message.reply_text(
            f"✅ **درخواست برداشت ثبت شد**\n\n"
            f"🐶 `{amount:,.2f} DOGS`\n"
            f"🆔 `#{request_id}`\n\n"
            f"⏳ در انتظار بررسی مالک.",
            parse_mode="Markdown",
            reply_markup=main_keyboard(
                user.id
            )
        )

        await send_owner_request(
            context,
            request_id,
            "withdraw",
            user,
            amount,
            text
        )

        return

    # =====================================================
    # SUPPORT
    # =====================================================

    if action == "user_support":

        states.pop(
            user.id,
            None
        )

        try:

            await context.bot.send_message(
                OWNER_ID,
                f"📩 **پیام کاربر**\n\n"
                f"👤 ID: `{user.id}`\n"
                f"👤 @{user.username or 'ندارد'}\n\n"
                f"{text}",
                parse_mode="Markdown"
            )

            await update.message.reply_text(
                "✅ پیام برای مالک ارسال شد.",
                reply_markup=main_keyboard(
                    user.id
                )
            )

        except Exception:

            await update.message.reply_text(
                "❌ ارسال پیام انجام نشد.",
                reply_markup=main_keyboard(
                    user.id
                )
            )

        return

    # =====================================================
    # ADMIN CONTACT TARGET
    # =====================================================

    if action == "admin_contact_target":

        if not is_owner(user.id):

            states.pop(
                user.id,
                None
            )

            return

        try:

            target_id = int(text)

        except Exception:

            await update.message.reply_text(
                "❌ ID باید عددی باشد.",
                reply_markup=cancel_keyboard()
            )

            return

        states[user.id] = {
            "action": "admin_contact_message",
            "target_id": target_id
        }

        await update.message.reply_text(
            "✉️ پیام را ارسال کنید.",
            reply_markup=cancel_keyboard()
        )

        return

    # =====================================================
    # ADMIN CONTACT MESSAGE
    # =====================================================

    if action == "admin_contact_message":

        if not is_owner(user.id):
            return

        target_id = state[
            "target_id"
        ]

        try:

            await context.bot.send_message(
                target_id,
                f"📩 پیام مدیریت:\n\n{text}"
            )

            await update.message.reply_text(
                "✅ پیام ارسال شد.",
                reply_markup=admin_keyboard()
            )

        except Exception:

            await update.message.reply_text(
                "❌ ارسال پیام انجام نشد.",
                reply_markup=admin_keyboard()
            )

        states.pop(
            user.id,
            None
        )

        return

    # =====================================================
    # ADMIN ADD USER
    # =====================================================

    if action == "admin_add_user":

        if not is_owner(user.id):
            return

        try:

            target_id = int(text)

        except Exception:

            await update.message.reply_text(
                "❌ ID باید عددی باشد.",
                reply_markup=cancel_keyboard()
            )

            return

        states[user.id] = {
            "action": "admin_add_amount",
            "target_id": target_id
        }

        await update.message.reply_text(
            "🐶 مقدار DOGS را وارد کنید:",
            reply_markup=cancel_keyboard()
        )

        return

    # =====================================================
    # ADMIN ADD AMOUNT
    # =====================================================

    if action == "admin_add_amount":

        if not is_owner(user.id):
            return

        try:

            amount = float(
                text.replace(",", "")
            )

            if amount <= 0:
                raise ValueError

        except Exception:

            await update.message.reply_text(
                "❌ عدد معتبر وارد کنید.",
                reply_markup=cancel_keyboard()
            )

            return

        target_id = state[
            "target_id"
        ]

        # اگر کاربر قبلاً ثبت نشده باشد
        if not user_exists(target_id):

            await update.message.reply_text(
                "❌ این کاربر هنوز ربات را Start نکرده است.",
                reply_markup=admin_keyboard()
            )

            states.pop(
                user.id,
                None
            )

            return

        if change_balance(
            target_id,
            amount
        ):

            new_balance = get_balance(
                target_id
            )

            await update.message.reply_text(
                f"✅ موجودی شارژ شد.\n\n"
                f"👤 `{target_id}`\n"
                f"🐶 +{amount:,.2f} DOGS\n"
                f"💰 موجودی جدید: `{new_balance:,.2f}`",
                parse_mode="Markdown",
                reply_markup=admin_keyboard()
            )

            try:

                await context.bot.send_message(
                    target_id,
                    f"💰 موجودی شما افزایش یافت.\n\n"
                    f"🐶 +{amount:,.2f} DOGS\n"
                    f"💰 موجودی جدید: "
                    f"{new_balance:,.2f} DOGS"
                )

            except Exception:
                pass

        else:

            await update.message.reply_text(
                "❌ عملیات انجام نشد.",
                reply_markup=admin_keyboard()
            )

        states.pop(
            user.id,
            None
        )

        return

    # =====================================================
    # ADMIN REMOVE USER
    # =====================================================

    if action == "admin_remove_user":

        if not is_owner(user.id):
            return

        try:

            target_id = int(text)

        except Exception:

            await update.message.reply_text(
                "❌ ID باید عددی باشد.",
                reply_markup=cancel_keyboard()
            )

            return

        states[user.id] = {
            "action": "admin_remove_amount",
            "target_id": target_id
        }

        await update.message.reply_text(
            "🐶 مقدار DOGS برای کسر را وارد کنید:",
            reply_markup=cancel_keyboard()
        )

        return

    # =====================================================
    # ADMIN REMOVE AMOUNT
    # =====================================================

    if action == "admin_remove_amount":

        if not is_owner(user.id):
            return

        try:

            amount = float(
                text.replace(",", "")
            )

            if amount <= 0:
                raise ValueError

        except Exception:

            await update.message.reply_text(
                "❌ عدد معتبر وارد کنید.",
                reply_markup=cancel_keyboard()
            )

            return

        target_id = state[
            "target_id"
        ]

        if change_balance(
            target_id,
            -amount
        ):

            new_balance = get_balance(
                target_id
            )

            await update.message.reply_text(
                f"✅ موجودی کسر شد.\n\n"
                f"👤 `{target_id}`\n"
                f"🐶 -{amount:,.2f} DOGS\n"
                f"💰 موجودی جدید: `{new_balance:,.2f}`",
                parse_mode="Markdown",
                reply_markup=admin_keyboard()
            )

            try:

                await context.bot.send_message(
                    target_id,
                    f"💰 از موجودی شما کسر شد.\n\n"
                    f"🐶 -{amount:,.2f} DOGS\n"
                    f"💰 موجودی جدید: "
                    f"{new_balance:,.2f} DOGS"
                )

            except Exception:
                pass

        else:

            await update.message.reply_text(
                "❌ کاربر پیدا نشد یا موجودی کافی نیست.",
                reply_markup=admin_keyboard()
            )

        states.pop(
            user.id,
            None
        )

        return

    # =====================================================
    # TRANSFER OWNER
    # =====================================================

    if action == "transfer_owner":

        if not is_owner(user.id):

            states.pop(
                user.id,
                None
            )

            return

        try:

            new_owner = int(text)

        except Exception:

            await update.message.reply_text(
                "❌ ID باید عددی باشد.",
                reply_markup=cancel_keyboard()
            )

            return

        states.pop(
            user.id,
            None
        )

        await update.message.reply_text(
            f"👑 مالک فعلی در کد ثابت است:\n"
            f"`{OWNER_ID}`\n\n"
            f"برای انتقال دائمی، OWNER_ID را به `{new_owner}` تغییر بده.",
            parse_mode="Markdown",
            reply_markup=admin_keyboard()
        )

        return


# =========================================================
# MEDIA HANDLER
# =========================================================

async def media_handler(
    update,
    context
):

    user = update.effective_user

    register_user(user)

    state = states.get(
        user.id
    )

    if not state:

        await update.message.reply_text(
            "از منوی ربات استفاده کنید.",
            reply_markup=main_keyboard(
                user.id
            )
        )

        return

    if state.get(
        "action"
    ) != "deposit_proof":

        await update.message.reply_text(
            "❌ در این مرحله فایل قابل قبول نیست.",
            reply_markup=cancel_keyboard()
        )

        return

    amount = state[
        "amount"
    ]

    deposit_format = state[
        "deposit_format"
    ]

    if update.message.photo:

        proof = "[IMAGE]"

    elif update.message.document:

        proof = "[DOCUMENT]"

    else:

        proof = "[FILE]"

    conn = db()

    conn.execute(
        """
        INSERT INTO requests
        (
            user_id,
            type,
            amount,
            username,
            proof,
            status,
            created_at
        )
        VALUES (
            ?,
            'deposit',
            ?,
            ?,
            ?,
            'pending',
            ?
        )
        """,
        (
            user.id,
            amount,
            user.username or "",
            proof,
            datetime.now().isoformat()
        )
    )

    conn.commit()

    request_id = conn.execute(
        "SELECT last_insert_rowid()"
    ).fetchone()[0]

    conn.close()

    states.pop(
        user.id,
        None
    )

    await update.message.reply_text(
        f"✅ **درخواست واریز ثبت شد**\n\n"
        f"🐶 `{amount:,.2f} DOGS`\n"
        f"📋 `{deposit_format}`\n"
        f"🆔 `#{request_id}`\n\n"
        f"⏳ در انتظار بررسی مالک.",
        parse_mode="Markdown",
        reply_markup=main_keyboard(
            user.id
        )
    )

    await send_owner_request(
        context,
        request_id,
        "deposit",
        user,
        amount,
        proof
    )


# =========================================================
# OWNER REQUEST
# =========================================================

async def send_owner_request(
    context,
    request_id,
    request_type,
    user,
    amount,
    proof
):

    if request_type == "deposit":

        title = "💳 **واریز جدید**"

        deposit_format = (
            f"{DEPOSIT_PREFIX} "
            f"{amount:g} DOGS "
            f"{DEPOSIT_WALLET}"
        )

        extra = (
            f"📋 فرمت واریز:\n"
            f"`{deposit_format}`\n\n"
        )

    else:

        title = "💸 **برداشت جدید**"

        extra = ""

    text = (
        f"{title}\n\n"
        f"🆔 درخواست: `#{request_id}`\n"
        f"👤 User ID: `{user.id}`\n"
        f"👤 Username: @{user.username or 'ندارد'}\n"
        f"🐶 مبلغ: `{amount:,.2f} DOGS`\n\n"
        f"{extra}"
        f"📎 مدرک / اطلاعات:\n"
        f"{proof}"
    )

    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                "✅ تأیید",
                callback_data=f"approve_{request_id}"
            ),
            InlineKeyboardButton(
                "❌ رد",
                callback_data=f"reject_{request_id}"
            )
        ]
    ])

    try:

        await context.bot.send_message(
            OWNER_ID,
            text,
            parse_mode="Markdown",
            reply_markup=keyboard
        )

    except Exception as e:

        logger.error(
            "OWNER SEND ERROR: %s",
            e
        )


# =========================================================
# ERROR
# =========================================================

async def error_handler(
    update,
    context
):

    logger.error(
        "BOT ERROR",
        exc_info=context.error
    )


# =========================================================
# MAIN
# =========================================================

def main():

    if not BOT_TOKEN:

        raise RuntimeError(
            "BOT_TOKEN در Environment Variables تنظیم نشده است."
        )

    init_db()

    # API در یک Thread جدا اجرا می‌شود
    api_thread = threading.Thread(
        target=start_api_server,
        daemon=True
    )

    api_thread.start()

    application = (
        Application.builder()
        .token(BOT_TOKEN)
        .build()
    )

    application.add_handler(
        CommandHandler(
            "start",
            start
        )
    )

    application.add_handler(
        CommandHandler(
            "admin",
            admin_command
        )
    )

    application.add_handler(
        CallbackQueryHandler(
            callbacks
        )
    )

    application.add_handler(
        MessageHandler(
            filters.PHOTO |
            filters.Document.ALL,
            media_handler
        )
    )

    application.add_handler(
        MessageHandler(
            filters.TEXT &
            ~filters.COMMAND,
            text_handler
        )
    )

    application.add_error_handler(
        error_handler
    )

    logger.info(
        "LIMBO DOGS BOT STARTED"
    )

    application.run_polling(
        drop_pending_updates=True
    )


# =========================================================
# RUN
# =========================================================

if __name__ == "__main__":
    main()
